...
class MIPSRMPGroup(
    ...
    def _learn(
        ...
        # Lexicographic order
        lexicographic_order = [
            [0] + [profile + 1 for profile in lexicographic_order[dm]] for dm in DM
        ]
        ...
        for dm in DM:
            for index in preference_relations_indices[dm]:
                if not self.inconsistencies:
                    self.prob += s[dm][index][lexicographic_order[dm][0]] == 1
                self.prob += s[dm][index][lexicographic_order[dm][k]] == 0
        ...
        for h in profile_indices:
            ...
            for dm in DM:
                for index, relation in enumerate(preference_relations[dm]):
                    ...
                    self.prob += lpSum(
                        [omega[dm][a][lexicographic_order[dm][h]][j] for j in M]
                    ) >= (
                        lpSum([omega[dm][b][lexicographic_order[dm][h]][j] for j in M])
                        + self.gamma
                        - s[dm][index][lexicographic_order[dm][h]] * (1 + self.gamma)
                        - (1 - s[dm][index][lexicographic_order[dm][h - 1]])
                    )
                    ...
                    self.prob += lpSum(
                        ...
                        lpSum([omega[dm][b][lexicographic_order[dm][h]][j] for j in M])
                        - (1 - s[dm][index][lexicographic_order[dm][h]])
                        - (1 - s[dm][index][lexicographic_order[dm][h - 1]])
                    )
                ...
                for index, relation in enumerate(indifference_relations[dm]):
                    ...
                    if not self.inconsistencies:
                        self.prob += lpSum(
                            ...
                            [omega[dm][b][lexicographic_order[dm][h]][j] for j in M]
                        )
                    else:
                        self.prob += lpSum(
                            [omega[dm][a][lexicographic_order[dm][h]][j] for j in M]
        ...
        return group_model(ModelEnum.SRMP, shared_params)(
            ...
            lexicographic_order=[  # type: ignore
                [p - 1 for p in lexicographic_order[dm][1:]] for dm in DM
            ],
        )
